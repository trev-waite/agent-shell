...
function createMockStore(initial: RelayEvent[] = []): ExecutionStore {
  ...
  return {
    ...
    getSession(sessionId: string) {
      ...
      return Promise.resolve({ id: sessionId, prompt: "original prompt", createdAt: Date.now() });
    },
    listSessions() {
      return Promise.resolve([{ id: "sess-1", prompt: "original prompt", createdAt: Date.now() }]);
    },
    events: {
...
describe("Runtime.rerun", () => {
  test("resumes from the latest checkpoint without re-emitting the user message", async () => {
    ...
    const allEvents = await store.events.getBySession("sess-1");
